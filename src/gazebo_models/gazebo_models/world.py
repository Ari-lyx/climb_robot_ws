...
def generate_world(directory, config, mode):
    ...
    output.write_text(f'''<?xml version="1.0"?><sdf version="1.6"><world name="climb_world">
      ...
      <physics name="ode" type="ode"><max_step_size>{physics['step_size']}</max_step_size><real_time_update_rate>{physics['update_rate']}</real_time_update_rate><ode><solver><type>{physics.get("solver_type", "quick")}</type><iters>{physics['solver_iterations']}</iters><sor>1.0</sor></solver><constraints><cfm>{physics.get("constraint_cfm", 0.000001)}</cfm><erp>0.2</erp><contact_max_correcting_vel>0.1</contact_max_correcting_vel><contact_surface_layer>0.0005</contact_surface_layer></constraints></ode></physics>
